...
def load_torchvision_data(
    ...
    if splits is None and valid_size == 0:
        ...
        train_idx, valid_idx = random_index_split(
            len(train), 1 - valid_size, (maxsize, None)
...
def load_imagenet(datadir=None, resize=None, tiny=False, augmentations=False, **kwargs):
    ...
    if resize is not None:
        train_transform_list.insert(3, transforms.Resize((resize, resize)))
        valid_transform_list.insert(2, transforms.Resize((resize, resize)))
    ...
    valid_data = dset.ImageFolder(
        validdir,
        transforms.Compose(valid_transform_list),
    )
    fold_loaders, dsets = load_torchvision_data(
        "Imagenet", transform=[], data=(train_data, valid_data), **kwargs
    )
...
def load_textclassification_data(
    dataname,
    vecname="glove.42B.300d",
    shuffle=True,
    random_seed=None,
    num_workers=0,
    preembed_sentences=True,
    loading_method="sentence_transformers",
    device="cpu",
    ...
    def batch_processor_tt(
        batch, TEXT=None, sentemb=None, return_lengths=True, device=None
    ):
        ...
        texttensor = torch.nn.utils.rnn.pad_sequence(
            texts, batch_first=True, padding_value=pad_idx
        )
        if sentemb:
    ...
    if loading_method == "torchtext":
        ...
        if preembed_sentences:
            ...
            sentembedder = BoWSentenceEmbedding(
                vsize, edim, text_field.vocab.vectors, pidx
            )
            batch_processor = partial(
                batch_processor_tt,
                TEXT=text_field,
                sentemb=sentembedder,
                return_lengths=False,
            )
        else:
            batch_processor = partial(
                batch_processor_tt, TEXT=text_field, return_lengths=True
            )
    ...
    dataloader_args = dict(
        batch_size=batch_size, num_workers=num_workers, collate_fn=batch_processor
    )
    train_loader = dataloader.DataLoader(
        train, sampler=train_sampler, **dataloader_args
    )
    valid_loader = dataloader.DataLoader(
        train, sampler=valid_sampler, **dataloader_args
    )
    dataloader_args["shuffle"] = False
    test_loader = dataloader.DataLoader(test, **dataloader_args)
    ...
    if print_stats:
        ...
        print(
            "Fold Sizes: {}/{}/{} (train/valid/test)".format(
                len(train_idx), len(valid_idx), len(test)
            )
        )
...
def combine_datasources(
    ...
    train_idx, valid_idx = random_index_split(
        len(dset), 1 - valid_size, (maxsize, None)
    )  # No maxsize for validation
    ...
    train_loader_ext = dataloader.DataLoader(
        merged_dset, sampler=train_sampler, **dataloader_args
    )
    valid_loader_ext = dataloader.DataLoader(
        merged_dset, sampler=valid_sampler, **dataloader_args
    )
