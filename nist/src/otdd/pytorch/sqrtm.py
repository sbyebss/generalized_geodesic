...
def symsqrt_v2(A, func="symeig"):
    ...
    sol = torch.matmul(
        torch.matmul(v, torch.diag_embed(s.sqrt(), dim1=-2, dim2=-1)),
        v.transpose(-2, -1),
    )
...
def sqrtm_newton_schulz(
    A, numIters, reg=None, return_error=False, return_inverse=False
):
    ...
    I = (
        torch.eye(dim, dim).view(1, dim, dim).repeat(batchSize, 1, 1).to(A.device)
    )  # .type(dtype)
    Z = (
        torch.eye(dim, dim).view(1, dim, dim).repeat(batchSize, 1, 1).to(A.device)
    )  # .type(dtype)
...
def create_symm_matrix(
    batchSize, dim, numPts=20, tau=1.0, dtype=torch.float32, verbose=False
):
    ...
    if verbose:
        print(
            "Creating batch %d, dim %d, pts %d, tau %f, dtype %s"
            % (batchSize, dim, numPts, tau, dtype)
        )
    return A
