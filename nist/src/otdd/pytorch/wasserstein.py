...
def bbures_distance(
    ...
    if sqrtΣ1 is None and not diagonal_cov:
        sqrtΣ1 = (
            ...
            else sqrtm_newton_schulz(Σ1, sqrt_niters)
        )  # , return_inverse=True)
    ...
    if diagonal_cov:
        ...
        sqrtΣ2 = (
            sqrtm(Σ2)
            if sqrt_method == "spectral"
            else sqrtm_newton_schulz(Σ2, sqrt_niters)
        )
        ...
        if sqrt_method == "spectral":
            ...
            cross = sqrtm_newton_schulz(
                torch.matmul(torch.matmul(sqrtΣ1, Σ2), sqrtΣ1), sqrt_niters
            )
...
def wasserstein_gauss_distance(
    ...
    if cost_function == "euclidean":
        mean_diff = ((μ_1 - μ_2) ** 2).sum(
            axis=-1
        )  # I think this is faster than torch.norm(μ_1-μ_2)**2
    else:
        mean_diff = cost_function(μ_1, μ_2)
        pdb.set_trace(
            header="TODO: what happens to bures distance for embedded cost function?"
        )
...
def pwdist_gauss(
    M1, S1, M2, S2, symmetric=False, return_dmeans=False, nworkers=1, commute=False
):
    """POTENTIALLY DEPRECATED.
    ...
    if nworkers > 1:
        results = Parallel(n_jobs=nworkers, verbose=1, backend="threading")(
            delayed(wasserstein_gauss_distance)(
                M1[i], M2[j], S1[i], S2[j], squared=True
            )
            for i, j in pairs
        )
...
def efficient_pwdist_gauss(
    ...
    if (both_sqrt and sqrt_pref == 0) or (sqrtS1 is not None):
        ...
        S2
    ):  # No precomputed squareroots provided. Compute, but choose smaller of the two!
        flip = False
        S = S1
    else:
        flip = True
        ...
        for i, Σ in tqdm(enumerate(S), leave=False):
            if diagonal_cov:
                ...
                sqrtS.append(
                    sqrtm(Σ)
                    if sqrt_method == "spectral"
                    else sqrtm_newton_schulz(Σ, sqrt_niters)
                )
...
def pwdist_exact(
    ...
    if loss == "sinkhorn":
        ...
        def distance(Xa, Xb):
            ...
            return torch.tensor(
                ot.emd2(ot.unif(Xa.shape[0]), ot.unif(Xb.shape[0]), C)
            )  # , verbose=True)
