...
class OnlineStatsRecorder:
    ...
    def update(self, batch):
        ...
        if self.n == 0:
            ...
            if self.diagonal_cov and self.centered_cov:
                ...
                self.Σ = ((batch - self.μ).T).matmul(batch - self.μ) / (
                    1.0 * self.n - 1
                )
            else:
            ...
            if self.diagonal_cov and self.centered_cov:
                self.Σ = (
                    (m - 1) * self.Σ
                    + ((m - 1) / (m + n - 1)) * ((batch - self.μ).pow(2).sum(axis=0))
                ) / (m + n - 1)
            elif self.diagonal_cov and not self.centered_cov:
                self.Σ = (m - 1) / (m + n - 1) * self.Σ + 1 / (m + n - 1) * (
                    batch.pow(2).sum(axis=0)
                )
            elif self.centered_cov:
                self.Σ = (
                    (m - 1) * self.Σ
                    + ((m - 1) / (m + n - 1))
                    * ((batch - self.μ).T).matmul(batch - self.μ)
                ) / (m + n - 1)
            else:
                self.Σ = (m - 1) / (m + n - 1) * self.Σ + 1 / (m + n - 1) * (
                    batch.T
    ...
    def retrieve(self, verbose=False):
        ...
        if self.centered_cov:
            ...
            Σ = torch.nn.functional.relu(
                Σ
            )  # To avoid negative variances due to rounding
            return self.μ, Σ
        else:
...
def compute_label_stats(
    ...
    if to_tensor:
        ...
        M = torch.stack(
            [μ.to(device) for i, μ in sorted(M.items()) if μ is not None], dim=0
        )
        S = torch.stack(
            [Σ.to(device) for i, Σ in sorted(S.items()) if Σ is not None], dim=0
        )
    ...
    if eigen_correction:
        ...
        for i in range(S.shape[0]):
            if eigen_correction == "constant":
                ...
                S[i] += torch.diag(
                    λ * torch.ones(S.shape[1], device=device).uniform_(0.99, 1.01)
                )
                ...
                if s_min <= 1e-10:
                    S[i] += torch.diag(
                        λ * torch.abs(s_min) * torch.ones(S.shape[1], device=device)
                    )
...
def dimreduce_means_covs(Means, Covs, redtype="diagonal"):
    ...
    if redtype == "diagonal":
        ...
        Covs = [U, V]
        print(
            "Done! Σ's Dimensions: {} (Task 1) and {} (Task 2)".format(
...
def pairwise_distance_mse(U, V, D, reg=1):
    ...
    l = torch.norm(D - d_uv) ** 2 / D.numel() + reg * (
