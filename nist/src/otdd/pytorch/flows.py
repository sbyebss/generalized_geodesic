...
from .utils import inverse_normalize, register_gradient_hook, show_grid
...
class GradientFlow:
    ...
    def _add_callback(self, cb):
        ...
        self.compute_coupling = (
            cb.compute_coupling if hasattr(cb, "compute_coupling") else False
        )
        self.store_trajectories = (
            cb.store_trajectories if hasattr(cb, "store_trajectories") else False
        )
    ...
    def animate(self, display_freq=None, save_path=None, **kwargs):
        ...
        """
        cb = Plotting2DCallback(
            animate=True, display_freq=display_freq, save_path=save_path, **kwargs
        )
...
class OTDD_Gradient_Flow(GradientFlow):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            not (objective_type in ["otdd_only", "ot_only", "mixed"])
            or self.Dt is not None
        ), "If objective contains distance, must provide tgt dataset"
        ...
        self.X1_init = None
        self.compute_coupling = (
            callback.compute_coupling
            if hasattr(callback, "compute_coupling")
            else False
        )
        self.store_trajectories = (
        ...
        self.trajectory_freq = (
            callback.trajectory_freq if hasattr(callback, "trajectory_freq") else 1
        )
        ...
        otdd_args["method"] = (
            "augmentation" if method == "xyaugm" else "precomputed_labeldist"
        )
        if "diagonal_cov" not in otdd_args:
            otdd_args["diagonal_cov"] = method == "xy_augm"

        assert not (
            (otdd_args["inner_ot_method"] == "exact")
            and (method in ["xy_augm", "xytied"])
        ), "If inner_ot_method == 'exact', then flow method cannot be '{}'.".format(
            method
        )
    ...
    def init(self):
        ...
        self.class_sizes = torch.unique(self.otdd.Y1, return_counts=True)[
            1
        ]  # torch.tensor([(self.otdd.Y1 == c).sum().item() for c in self.otdd.V1])
        ...
        if self.use_torchoptim:
            if self.optim == "sgd":
                optimizer = torch.optim.SGD(
                    flow_params, lr=self.step_size, momentum=0.5
                )
        ...
        if self.compute_coupling is not False:
    ...
    def coupling_update(self):
        ...
        with torch.no_grad():
            ...
            self.otdd.π = self.otdd.compute_coupling(
                entreg=self.entreg_π, method=otmethod, verbose=True, numItermax=50
            )
    ...
    def gradient_update(self):
        if self.use_torchoptim:
            ...
            if self.noisy_update:
                X += torch.randn(X.shape) * self.noise_β

            if self.method in ["xonly", "xonly-attached"]:
                [gX] = torch.autograd.grad(L_αβ, [X])
                self.otdd.X1.data -= lr * len(X) * gX
            elif self.method == "xytied":
                [gX, gM, gC] = torch.autograd.grad(
                    L_αβ, [X, self.otdd.Means[0], self.otdd.Covs[0]]
                )
                for g in [gX, gM, gC]:
                    if torch.isnan(g).any():
                        pdb.set_trace(header="Failed Grad Check")

                class_sizes = (
                    torch.tensor(
                        [(self.otdd.Y1 == c).sum().item() for c in self.otdd.V1]
                    )
                    * 0.1
                )

                self.otdd.X1.data -= lr * len(X) * gX
                self.otdd.Means[0].data -= (
                    lr * len(self.otdd.Means[0]) * gM / class_sizes.view(-1, 1)
                )  # * 0.2 # slower lr for Means and Covs
                self.otdd.Covs[0].data -= (
                    lr * len(self.otdd.Covs[0]) * gC / class_sizes.view(-1, 1, 1)
                )  # * 0.2
            elif self.method == "xyaugm":
                if self.noisy_update:
                    pdb.set_trace(header="Not sure xyaugm works with noisy update yet")
                [gXMC] = torch.autograd.grad(
                    L_αβ, [self.otdd.XμΣ1]
                )  # , self.otdd.Means[0], self.otdd.Covs[0]])
    ...
    def stats_update(self):
        ...
        if self.method == "xonly":
            Ds_t = TensorDataset(
                self.otdd.X1.detach().clone(), self.otdd.Y1
            )  # self.Ds.tensors[1])

            self.otdd.Means[0], self.otdd.Covs[0] = compute_label_stats(
                Ds_t,
            ...
            if self.clustering_method == "kmeans":
                ...
                L = (
                    ...
                    .fit(self.otdd.XμΣ1.detach().numpy()[:, d:])
                    .labels_
                )
            ...
            self.otdd.targets1 = self.otdd.Y1
            self.otdd.classes1 = [
                str(i.item()) for i in torch.unique(self.otdd.targets1)
            ]

            Ds_t = TensorDataset(self.otdd.X1, self.otdd.Y1)

            M, S = compute_label_stats(
                Ds_t,
                targets=self.otdd.targets1,
                indices=np.arange(len(self.otdd.Y1)),
                classnames=self.otdd.classes1,
    ...
    def step(self, iter):
        ...
        if self.store_trajectories and (iter % self.trajectory_freq == 0):
            self.Xt = torch.cat(
                [self.Xt, self.otdd.X1.detach().clone().cpu().float().unsqueeze(-1)],
                dim=-1,
            )
            self.Yt = torch.cat(
                [self.Yt, self.otdd.Y1.detach().clone().cpu().unsqueeze(-1)], dim=-1
            )
    ...
    def flow(self, tol=1e-3):
        ...
        for iter, t in enumerate(pbar, 1):
            ...
            obj = self.step(iter)
            logger.info(
                f"t={t:8.2f}, F(a_t)={obj:8.2f}"
            )  # Although things have been updated, this is obj of time t still
            self.history.append(obj)
            self.t = t
            self.callback.on_step_end(
                self, self.otdd, iter, t, obj
            )  # Now that we pass whole flow obj, maybe no need to pass otdd
            Δ = np.abs(obj - prev)
            if tol and (Δ < tol):
                logger.warning(
                    f"Stoping condition met (Δ = {Δ:2.2e} < {tol:2.2e} = tol). Terminating flow."
                )
                break
            else:
                prev = obj
...
class CallbackList(Callback):
    def __init__(self, cbs):
        ...
        trajfreq_attrs = [
            cb.trajectory_freq for cb in cbs if cb.trajectory_freq is not None
        ]
        self.trajectory_freq = (
            np.array(trajfreq_attrs).min() if trajfreq_attrs else None
        )
    ...
    def on_flow_begin(self, *args, **kwargs):
        for cb in self.cbs:
            cb.on_flow_begin(*args, **kwargs)

    def on_flow_end(self, *args, **kwargs):
        for cb in self.cbs:
            cb.on_flow_end(*args, **kwargs)

    def on_step_begin(self, *args, **kwargs):
        for cb in self.cbs:
            cb.on_step_begin(*args, **kwargs)

    def on_step_end(self, *args, **kwargs):
        for cb in self.cbs:
            cb.on_step_end(*args, **kwargs)
...
class Plotting2DCallback(Callback):
    def __init__(
        ...
        raise DeprecationWarning(
            "Plotting2DCallback has been deprecated in favor of PlottingCallback"
        )
    ...
    def _plot(self, otdd, X1, Y1=None, X2=None, Y2=None, title=None, trajectories=None):
        with torch.no_grad():
            ...
            if self.show_target and self.show_coupling:
                π = otdd.π
                plot2D_samples_mat(
                    X1, X2, π, ax=ax, linewidth=0.1, thr=1e-10, linestyle=":"
                )
            ax.set_xlim(xrng)
            ax.set_ylim(yrng)
            ax.set_title("")
    ...
    def on_step_end(self, flow, otdd, iteration, t, d, **kwargs):
        if self.display_freq is None or (iteration % self.display_freq == 0):  # display
            ...
            if self.animate:
                ...
                if self.save_path:
                    outpath = self.save_path + "t{}.{}".format(
                        iteration, self.save_format
                    )
                    plt.tight_layout()
                    plt.savefig(outpath, dpi=300)  # bbox_inches='tight',
                plt.show(block=False)
        ...
        with torch.no_grad():
            if self.animate:
                ...
                if self.ndim == 2:
                    ...
                    ax = fig.add_subplot(
                        111, azim=self.azim, elev=self.elev, projection="3d"
                    )
            ...
            if title is not None:
                if self.ndim == 2:
                    ax.text(
                        0.5, 1.1, title, transform=ax.transAxes, ha="center", size=18
                    )
                else:
                    ax.text2D(
                        0.5, 0.85, title, transform=ax.transAxes, ha="center", size=18
                    )
...
class Embedding2DCallback(Plotting2DCallback):
    ...
    def _embed(self, otdd):
        with torch.no_grad():
            X = torch.cat(
                [otdd.X1.detach().clone().cpu(), otdd.X2.clone().cpu()], dim=0
            )
            if tsnelib in ["sklearn", "tsnecuda"]:
                X_emb = TSNE(
                    n_components=self.ndim, verbose=0, perplexity=50
                ).fit_transform(X)
            else:
                if not hasattr(self, "tsne") or self.tsne is None:
                    X_emb = TSNE(
                        n_components=self.ndim, perplexity=50, n_jobs=8, verbose=True
                    ).fit(X)
                    self.tsne = X_emb
    ...
    def on_flow_begin(self, otdd, d):
        ...
        Ms_emb, Cs_emb = compute_label_stats(
            Ds_emb,
        ...
        Mt_emb, Ct_emb = compute_label_stats(
            Dt_emb,
        ...
        otdd_emb = otdd.copy(
            keep=["classes1", "classes2", "targets1", "targets2", "Y1", "Y2"]
        )
        ...
        if self.store_trajectories:
            self.Xt = (
                self.otdd_emb.X1.detach().clone().cpu().unsqueeze(-1).float()
            )  # time will be last dim
        ...
        if self.display_freq is None or (iteration % self.display_freq == 0):
            ...
            if self.store_trajectories:
                ...
                self.Xt = torch.cat(
                    [
                        self.Xt,
                        self.otdd_emb.X1.detach().clone().cpu().float().unsqueeze(-1),
                    ],
                    dim=-1,
                )
                self.Yt = torch.cat(
            ...
            super().on_step_end(
                ...
                t,
                d,
                trajectories=self.Xt
                if self.show_trajectories
                else None,  # Must override non-embedded trajectories
                **kwargs,
            )
...
class ImageGridCallback(Callback):
    ...
    def _plot(self, otdd, X1, X2, title):
        with torch.no_grad():
            batch = X1[self.indices].view(
                len(self.indices), self.channels, self.imdim[0], self.imdim[1]
            )
        ...
        if self.only_matched:
            ...
            for c in otdd.V1:
                ...
                if len(idxs_class_matched) >= self.nrow:
                    ...
                    p /= p.sum()
                    idxs_class_selected = np.sort(
                        np.random.choice(
                            idxs_class_matched, self.nrow, replace=False, p=p
                        )
                    )
                else:
                    ## Not enough matched, complete with unmatched
                    idxs_class = torch.where(Y1 == c)[0]
                    unmatched = np.random.choice(
                        idxs_class, self.nrow - len(idxs_class_matched), replace=False
                    )
                    idxs_class_selected = np.concatenate(
                        (idxs_class_matched, unmatched)
                    )
            ...
            idxs = np.sort(
                np.random.choice(X1.shape[0], self.nrow * self.ncol, replace=False)
            )
        else:
            idxs = []
            for c in otdd.V1:  # V1 is never index-shifted, so this works
                idxs_class = torch.where(Y1 == c)[0]
                idxs.append(
                    np.sort(np.random.choice(idxs_class, self.ncol, replace=False))
                )
            idxs = np.concatenate(idxs)
        self.indices = idxs
    ...
    def on_flow_begin(self, otdd, d):
        ...
        if self.animate:
            ...
            if self.save_path:
                ...
                plt.savefig(
                    outpath + ".pdf", dpi=300, transparent=self.transparent
                )  # bbox_inches='tight',
                plt.savefig(
                    outpath + ".png", dpi=300, transparent=self.transparent
                )  # bbox_inches='tight',
            plt.show(block=False)
            plt.pause(1)
            plt.close()
