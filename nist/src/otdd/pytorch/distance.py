...
class DatasetDistance:
    ...
    def _get_label_distances(self):
        ...
        if self.inner_ot_method == "gaussian_approx":
            ...
            self._pwlabel_stats_2 = {"dlabs": DYY2}
            self.pwlabel_stats = {"dlabs": DYY12}
    ...
    def distance(self, maxsamples=10000, return_coupling=False):
        ...
        if return_coupling:
            return (
                dist,
                π,
                self.X2[idxs_2].to(device_dists),
                self.Y2[idxs_2].to(device_dists),
            )
        else:
            return dist
...
class FeatureCost:
    ...
    def _batchify_computation(self, X, side="x", slices=20):
        if side == "x":
            out = torch.cat(
                [self.src_emb(b).to("cpu") for b in torch.chunk(X, slices, dim=0)]
            )
        else:
            out = torch.cat(
                [self.tgt_emb(b).to("cpu") for b in torch.chunk(X, slices, dim=0)]
            )
