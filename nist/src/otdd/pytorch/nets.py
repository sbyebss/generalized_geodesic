...
class MLPClassifier(nn.Module):
    ...
    def _init_classifier(self, num_classes=None):
        num_classes = self.num_classes if num_classes is None else num_classes
        self.classifier = nn.Sequential(
            nn.Linear(self.hidden_sizes[-1], 20), nn.ReLU(), nn.Linear(20, num_classes)
        )
...
class BoWSentenceEmbedding:
    def __init__(
        ...
        self.method = method
        if method == "bag":
            self.emb = nn.EmbeddingBag.from_pretrained(
                pretrained_vec, padding_idx=padding_idx
            )
        else:
...
class ConvPushforward(nn.Module):
    def __init__(
        self, input_size=28, channels=1, nlayers_conv=2, nlayers_mlp=3, **kwargs
    ):
...
class ConvPushforward2(nn.Module):
    def __init__(
        ...
        if input_size == 32:
            ...
            self.dnconv2 = nn.ConvTranspose2d(
                16, 8, 5, stride=3, padding=1
            )  # b, 8, 15, 15
            self.dnconv3 = nn.ConvTranspose2d(
                8, 1, 2, stride=2, padding=1
            )  # b, 1, 28, 28
        else:
            raise NotImplemented("Can only do LeNet on 28x28 or 32x32 for now.")
        self.feat_dim = feat_dim
