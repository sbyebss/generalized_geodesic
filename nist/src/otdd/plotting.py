...
def heatmap(
    data, row_labels, col_labels, ax=None, cbar=True, cbar_kw={}, cbarlabel="", **kwargs
):
...
def distance_scatter(d, topk=10, show=True, save_path=None):
    ...
    adjust_text(
        texts, force_text=0.05, arrowprops=dict(arrowstyle="-|>", color="r", alpha=0.5)
    )
...
def dist_adapt_regplot(
    ...
    if annotate:
        ...
        for i, a in df.iterrows():
            lab = (
                r"{}$\rightarrow${}".format(a.src, a.tgt)
    ...
    ax.set_title(
        r"Distance vs Adaptation" + (": {}".format(title) if title else ""),
...
def imshow_group_boundaries(ax, gU, gV, group_names, side="both", alpha=0.2, lw=0.5):
    ...
    if group_names:
        ...
        for i, y in enumerate(posy[:-1]):
            annotate_group(
                group_names[1][i], (posy[i], posy[i + 1]), ax, orient="h", side="top"
            )
        for i, x in enumerate(posx[:-1]):
            annotate_group(
                group_names[0][i], (posx[i], posx[i + 1]), ax, orient="v", side="right"
            )
