...
# TODO: If we find a way to produce more general commuting covariances, generalize beyond diagonal.
class OTDD_Gradient_Flow(GradientFlow):
    ...
    def __init__(self, src, tgt=None, objective_type = 'otdd_only', functional = None,
        ...
        #assert not (self.method == 'xyaugm') and ()
        ...
        # if step_size is None and steps is not None:
        #     self.times, self.step_size = np.linspace(0,1,steps+1, retstep=True)
        # elif step_size is not None and steps is None:
        #     self.times = np.arange(0,1,step_size).squeeze()
        ...
        if 'diagonal_cov' not in otdd_args:
            # Diagonal Covariance is mandatory for augmentation approach.
            otdd_args['diagonal_cov'] = (method == 'xy_augm')
        #print(otdd_args)

        # exact method doesn't have Means/Covs - xyaugm doesn't make sense
    ...
    def init(self):
        ...
        # Load datasets to create initial X,Y points.
        #self.otdd._load_datasets(10000, device=self.device)
        ### Also initial target Mean and Covs (not in computational graph, static)
        ###_, _ = self.otdd._get_label_stats(side='tgt')

        # Run distance one initial time to trigger constructions of objects needed by optimizer
        # This is a non-differentiable run, first step will compute another one.
        d = self.otdd.distance()
        ...
        # Require grads of things that require backpropping
        ...
        if self.method == 'xonly' or self.method == 'xonly-attached':
            ...
            # For xonly, μ, Σ are not in the (backward) computational graph
            # For xonly-attached X, μ, Σ are all in the comp graph
            # For inner_ot_method=='exact', there are no μ, Σ.
            # In any of these cases, only X gets updated by gradient
            ...
            # X, μ, Σ are all in the comp graph and all get updated by gradient
            flow_params = [self.otdd.X1, self.otdd.Means[0], self.otdd.Covs[0]]

        elif self.method == 'xyaugm':
            # X, μ, Σ are all in the comp graph and all get updated by gradient.
            # But they are all stacked into one vector so it's easier
        ...
        # if self.otdd.Covs[0].requires_grad:
        #     register_gradient_hook(self.otdd.X1, 'X1 Gradient Hook')
        #     register_gradient_hook(self.otdd.Covs[0], 'Covs Gradient Hook')
        #
    ...
    def coupling_update(self):
        ...
        with torch.no_grad():
            # Better to do it here, in case more than one callback needs it, to avoid duplicated computation
    ...
    def gradient_update(self):
        if self.use_torchoptim:
            # If we use torchoptim, optimizer already knows what params to update
            L_αβ = 0
            if self.objective_type in ['otdd_only', 'ot_only', 'mixed']:
                L_αβ += self.otdd.distance()
            ...
            if self.method in ['xonly', 'xonly-attached']:
                ...
                # METHOD 1: MANUALLY UPDATES MEANS AND VARIANCES WITH ORIGINAL LABELS
                # (i.e. ONLY GRADIENT in X, recompute Means and Covs for updated points)
                # METHOD 2: GRADIENT UPDATES MEANS AND VARIANCES - BUT KEEP THEM TIED
                ...
                #{c.item(): (flowobj.otdd.Y1 == c).sum().item() for c in flowobj.otdd.V1}
                ...
                # METHOD 3: GRADIENT UPDATES MEANS AND VARIANCES UNTIED VIA AUGMENTED REPR
    ...
    def label_update(self):
        ...
        # Update labels based on current per-point means/covs
        if cmethod == 'kmeans':
            # Kmeans requires specifying # of clusters - use # target
    ...
    def stats_update(self):
        ...
        if self.method == 'xonly':
            # METHOD 1.a: Requires recomputation of Means and Covs for updated points
            # Note that we dettach the X1's, so that in the next step there is no gradient
            # back to X from Means and Covariances.
            Ds_t = TensorDataset(self.otdd.X1.detach().clone(), self.otdd.Y1)#self.Ds.tensors[1])

            # Recompute label-cluster means and covariances using updated X's
            ...
            # METHOD 1.b: Requires recomputation of Means and Covs for updated points
            ...
            #self.otdd.Covs[0]  = C.data -> doing .data would through away grads

        elif self.method == 'xytied':
            # METHOD 2: No recomputation needed (all params are differentiable, no clustering)
            pass
        elif self.method == 'xyaugm':
            # METHOD 3: Must update X1,Y1 from updated augmented representation
            d = self.otdd.X1.shape[1]
    ...
    def step(self, iter):
        #self.callback.on_step_begin(self.otdd, t)
        assert self.initialized

        ##### Update differentiable-dynamic params
        # Force recomputation of stats, label-label distances before computing gradients
        ...
        ##### Update labels (if fixed_labels != False ) # TODO: Decide if before or after stats update
        ...
        #self.t = self.times[np.where(self.times==self.t)[0][0] + 1]
    ...
    def end(self):
        # Restore initial points to src dataset
        self.otdd.X1       = self.X1_init
        self.otdd.Y1       = self.Y1_init
        if self.otdd.inner_ot_method != 'exact':
            self.otdd.Means[0] = self.M1_init
            self.otdd.Covs[0]  = self.C1_init
        # for t in [self.otdd.X1, self.otdd.Y1, self.otdd.Means[0], self.otdd.Covs[0]]:
        #     t.requires_grad_(False)
        # OR alternatively clean up otdd (otdd.X1=None, otdd.X2=None etc)
...
class CallbackList(Callback):
    def __init__(self, cbs):
        ...
        # trjactories need to be stored if *any* callback requires them
...
class Plotting2DCallback(Callback):
    def __init__(self, display_freq=None, animate=False, entreg_π = 1e-4,
        ...
        # if self.animate:
        #     #assert self.flow_method == 'parametric'
    ...
    def _plot(self, otdd, X1, Y1=None, X2=None, Y2=None, title=None, trajectories=None):
        with torch.no_grad():
            ...
            # otmethod = 'emd' if self.entreg_π <= 1e-3 else 'sinkhorn_epsilon_scaling'
            # π = otdd.compute_coupling(entreg=self.entreg_π, method=otmethod,
            #                           verbose = True,  numItermax=50)
            if self.animate:
                ax = self.ax
    ...
    def on_flow_begin(self, otdd, d):
        ...
        #xrng, yrng = self._get_ax_ranges(otdd.X1,otdd.X2)
        if self.animate:
...
# TODO: Revisit Embedding2DCallback. THere's a lot of device conversion, pprobably unecessary.
class Embedding2DCallback(Plotting2DCallback):
    ...
    def _embed(self, otdd):
        ...
        #return X1_emb.to(otdd.X1.device), X2_emb.to(otdd.X2.device)
    ...
    def on_flow_begin(self, otdd, d):
        ...
        # Hacky: crate a new OTDD instance for the embedding.
        # Possible alternative: make plot_label_stats a staticmethod so that we can pass Means and Covs

        # # Approach 1:
        # Ds_emb.targets, Dt_emb.targets = otdd.targets1, otdd.targets2
        # Ds_emb.classes, Dt_emb.classes = otdd.classes1, otdd.classes2
        # otdd_emb = otdd.copy()
        # otdd_emb._init_data(Ds_emb, Dt_emb)

        # Approach 2: Make a full copy of otdd and just update what we'll need for plotting
        ...
        #_,_ = otdd_emb._get_label_stats()
        #_ = otdd_emb._get_label_distances()

        #super(Embedding2DCallback, self).on_flow_begin(self.otdd_emb, d)
        super().on_flow_begin(self.otdd_emb, d)

        # self._plot(otdd_emb, X1_emb, X2_emb, title)
        # if self.animate:
        #     self.camera.snap()
        # else:
        #     if self.save_path:
        #         outpath = self.save_path + '_t0.' + self.save_format
        #         plt.tight_layout()
        #         plt.savefig(outpath, dpi=300) #bbox_inches='tight',
        #     plt.show()
...
# TODO: Maybe plot also target domain samples (either randomly, or better yet,
# those mapped to source samples according to optimal coupling computed at time t=0
class ImageGridCallback(Callback):
    ...
    def _plot(self, otdd, X1, X2, title):
        with torch.no_grad():
            ...
            grid = make_grid(batch, nrow=self.ncol, padding=2, normalize=False,
        ...
        if self.only_matched:
            ...
            for c in otdd.V1:
                ...
                if len(idxs_class_matched) >= self.nrow:
                    ...
                    p = otdd.π[idxs_class_matched,:][:,Y2-min(Y2)==c].sum(axis=1).numpy().astype('float64')
                    p /= p.sum()
            ...
            for c in otdd.V1: # V1 is never index-shifted, so this works
                idxs_class = torch.where(Y1 == c)[0]
                idxs.append(np.sort(np.random.choice(idxs_class, min(self.ncol,len(idxs_class)), replace=False)))
            idxs = np.concatenate(idxs)
        self.indices = idxs
...
class TrainingCallback(Callback):
    ...
    def accuracy(output, target, topk=(1,)):
        ...
        with torch.no_grad():
            ...
            res = []
            for k in topk:
                correct_k = correct[:k].view(k,-1).float().sum() # TODO: 0, keepdim=True)
    ...
    def on_step_end(self, flow, otdd, iteration, t, d, **kwargs):
        ...
        #self.evaluate(net, otdd.X1.clone().detach(), otdd.Y1)
