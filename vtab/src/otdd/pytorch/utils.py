...
#from .datasets import DATASET_NORMALIZATION # TODO: find out why this import fails
...
def seed_everything(seed: int):
    ...
    np.random.seed(seed) # Numpy - cpu vars
    torch.manual_seed(seed)  # Torch - cpu vars
    torch.cuda.manual_seed(seed)  # Torch - gpu vars
...
# TODO: can we do this by extending / decorating the iter method
def safe_next_iter(iterable, iteree):
    """ Allows to call .next() on iterable when even when it runs out, by re-
    initializing it. An alternative to itertools.cycle """
    try:
        out = iterable.next()
    except:
        print('Reinitializing iterable')
        iterable = iter(iteree)
        out = iterable.next()
    return out,iterable

def infer_torch_object_type(d):
...
def extract_dataset_targets(d):
    ...
    if isinstance(d, torch.utils.data.dataset.Subset):
        dataset = d.dataset
        indices = d.indices
    # TODO: Add other types of datasets in https://pytorch.org/docs/stable/_modules/torch/utils/data/dataset.html?
    ...
    if (indices is None) and (targets is not None):
        ...
        # Some datasets have a dictionary of indices, e.g. H5Dataset
        indices = np.sort(np.array([v[1] for v in d.indices.values()]))
    else:
...
def extract_dataloader_targets(dl):
    ...
    # TODO: do we want these already in tensor instead of np array?
    if indices is None:
        indices = np.arange(len(targets))
    else:
        indices = np.sort(indices)
...
def load_full_dataset(data, targets=False, return_both_targets=False,
    ...
    if type(data) == dataloader.DataLoader:
        ...
        if maxsamples and len(data) > maxsamples:
            ...
            sampler = SubsetRandomSampler(idxs)
            loader = dataloader.DataLoader(data, sampler=sampler,
                                           batch_size=batch_size, num_workers=num_workers,
                                           pin_memory=pin_memory)
        else:
            ## No subsampling - preserve order (no shuffle)
            loader = dataloader.DataLoader(data, batch_size=batch_size,shuffle=False,
                                           num_workers=num_workers, pin_memory=pin_memory)
        # TODO: add num_wokers to these two?
    ...
    for batch in tqdm(loader, leave=False):
        ...
        #show_grid(make_grid(x[:64], nrow=8, padding=2, normalize=False, value_range=None, scale_each=False, pad_value=0))
        ...
        if feature_embedding is not None:
            ...
            try:
                ...
                # If this fails, it probabl means embedding is cpu, so no gain to be had
            except:
    ...
    if targets == 'infer':
        ...
        if force_label_alignment:
            # We force the values of the infered labels to align as well as possible to true labels
    ...
    if targets is False:
        ...
        if reindex: # FIXME: There must be a more efficient map-style way relabeling in pytorch!
...
def sample_kshot_task(dataset,k=10,valid=None):
    ...
    """
    inds_train = []
    # if type(dataset) is SubsetFromLabels: #torch.utils.data.dataset.Subset:
...
def extract_torchmeta_task(cs, class_ids):
    ...
    #ds.classes = torch.unique(ds_train.targets)
...
def choose_batch_sizes(n, m, maxbatch, minbatch, mode='dynamic', nbatches=None):
    """
        Args:
            - n (int): number of samples in dataset A
            - m (int): number of samples in dataset B
            - maxbatch (int): maximum batch size allowed
            - minbatch (int): minimum batch size allowed
            - mode (str): one of ['dynamic', 'min', 'max', 'fixed']
        Output:
            - NB_a: number of batches from dataset a
            - B_a: batch size for dataset a
            - drop_last_a: whether to drop last batch of dataset a
    """
    if mode == 'dynamic' and max(n/maxbatch,m/maxbatch) <= min(n/minbatch,m/minbatch):
        ...
        NB  = math.ceil(max(n/maxbatch,m/maxbatch))
        B_a, B_b = int(n/NB), int(m/NB)
    else:
        # Same batch size, number determined by number of samples or pre-specified
        B_a = min(maxbatch, n) # Batch can't be larger than dataset!
        B_b = min(maxbatch, m) # Batch can't be larger than dataset!
        # Only include last batch if it's above minB threshold
        NB_a = int(n/B_a) + (n % B_a >= minbatch)
        NB_b = int(m/B_b) + (m % B_b >= minbatch)
        if mode == 'min':
            NB = min(NB_a, NB_b) #len(dla), len(dlb))
        elif mode == 'max':
            NB = max(NB_a, NB_b)
        elif nbatches and type(nbatches) is int:
            NB = nbatches
    ...
    drop_last_a = (n % B_a > 0) and (n % B_a < minbatch)
    drop_last_b = (m % B_b > 0) and (m % B_b < minbatch)
    # Effective number of batches
    NB_a = int(n/B_a) + int(n % B_a > 0 and not drop_last_a)
    NB_b = int(m/B_b) + int(m % B_b > 0 and not drop_last_b)

    return B_a, B_b, NB_a, NB_b, drop_last_a, drop_last_b
