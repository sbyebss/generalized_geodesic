...
class OnlineStatsRecorder:
    ...
    def compute_from_loader(self, dataloader):
        ...
        if self.twopass:
            ...
            # On second pass, no need to recenter,
            # since we're already kind of doing that.
            # If this is changed to True, must be careful with how we retrieve.
            for x, _ in dataloader:
                x = x.type(self.dtype).to(device)
                x = self.embedding(x).detach() if self.embedding is not None else x
    ...
    def update(self, batch):
        ...
        if self.n == 0:
            ...
            ### Mean Update
            #self.μ = m/(m+n)*self.μ + n/(m+n)*batch.mean(dim=0)
            self.μ =  self.μ + (batch-self.μ).sum(axis=0)/(m+n)   # Stable Algo

            ### Cov Update
            # TODO: should we change these to in place updates to avoid creating new objects??
            if self.diagonal_cov and self.centered_cov:
...
def _single_label_stats(data, i, c, label_indices, M=None, S=None, batch_size=256,
    ...
    if len(label_indices) < 2:
        ...
        if type(data) == dataloader.DataLoader:
            ...
            #filtered_loader = dataloader.DataLoader(data.dataset, batch_size=batch_size,sampler=SubsetRandomSampler(label_indices))
        else:
        ...
        if online:
            ...
            # μ2 = torch.mean(X, dim = 0).flatten()
            # Σ2 = cov(X.view(X.shape[0], -1).t())
            #print((Σ-Σ2).mean())
            #print((μ-μ2).mean())
            #pdb.set_trace()
        else:
...
def compute_label_stats(data, targets=None,indices=None,classnames=None,
    ...
    # TODO: Have a way to provide effective_targets directly?
    if targets is None:
    ...
    if nworkers > 1: # TODO: Decide if we want to keep this or not
        import torch.multiprocessing as mp # Ugly, sure. But useful.
        mp.set_start_method('spawn',force=True)
        # VERSION 1: Works now both in CPU and GPU
        # TODO: right now number of processes is number of classes. Change to
        # actually use nworkers var. Might require Pool.
        ...
        # results = [pool.apply(_single_label_stats, args=(i,c,allindices,targets,data,M,S)) for (i,c) in enumerate(classes)]
    else:
        for i,c in enumerate(classnames):
            ...
            # if diagonal_cov:
            #     Σ = Σ.diag()
            #     Σ[Σ<0] = 0 # Sometimes we get very small negative epsilon values here.
            #                # Better solution: don't compute Σ with sqrtm, do it variance per frature inside _single_label_stats
            M[i],S[i] = μ, Σ
    ...
    # if type(data) == dataloader.DataLoader and hasattr(data.sampler,'indices'):
    #     # Reinstante original indices in sampler
    #     data.sampler.indices = allindices
