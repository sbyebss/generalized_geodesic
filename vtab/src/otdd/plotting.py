...
import pdb # TODO: clean

logger = logging.getLogger(__name__) # DEBUG: Fix
# DEBUG: Fix
# TODO: Fix
...
def gaussian_density_plot(P=None, X=None, method = 'exact', nsamples = 1000,
    ...
    if len(levels) < 2:
        #pdb.set_trace()
        # This can happen if e.g. np.amax(f) < 2*step.
        # Would cause countourf to break, so add a smaller one
...
# Ported from https://matplotlib.org/3.1.1/gallery/images_contours_and_fields/image_annotated_heatmap.html
...
def distance_scatter(d, topk=10, show=True, save_path =None):
    ...
    if type(keys[0]) is tuple and len(keys[0]) == 2:
        # We passed the name of both tasks
    ...
    for i, (x, y, name) in enumerate(zip(x_coord,dists,keys)):
        ...
        #if not set(V1).intersection(set(V2)) and (i< 0.92*len(keys) or i > 0.999*len(keys)):
...
def dist_adapt_joinplot(df, yvar='delta', show=True, type='joinplot', save_path = None):
    #direction = {'Delta_A': 'B⟶A','Delta_B': 'A⟶B', 'Delta_AB': 'A⟷B'}
...
def dist_adapt_regplot(df, yvar, xvar='dist', xerrvar=None, yerrvar=None,
    ...
    #sns.regplot(x=xvar, y=yvar, data=df, ax = ax, fit_reg=False, color=color)
    #sns.regplot(x=xvar, y=yvar, data=df, ax = ax, scatter=False, color=color, label=legend_label)
...
def annotate_group(name, span, ax=None, orient='h', side=None):
    ...
    #if ((orient == 'h' and side == 'top') or (orient == 'v' and side == 'left')):
    if side == 'bottom':
    ...
    #     lim = lims[0]
    #     arrow_coord = lim + 0.02*range if orient == 'h' else lim - 0.02*range
    #     #text_pad = 0.02*range #if orient == 'h' else 0.2
    # else:
    #     lim = lims[1]
    #     arrow_coord = lim - 0.02*range if orient == 'h' else lim + 0.02*range
    #     #text_pad = -0.02*range #if orient == 'h' else 0.2
    ...
    # text_pad = 0.02*range
    # text_pad = -text_pad if ((orient == 'h' and side == 'top') or \
    #                          (orient == 'v' and side == 'left')) else text_pad

    #min = lim + 0.5 if orient == 'h' else lim - 0.5 #(0.5 if orient =='h' else 0.5)
    center = np.mean(span)
