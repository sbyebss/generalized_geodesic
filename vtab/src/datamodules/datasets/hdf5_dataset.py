...
class H5Dataset(Dataset):
    def __init__(
        ...
        self.targets = torch.cat(
            [
                torch.from_numpy(self._archives[a][target_name][:])
                for a in range(len(self._archives))
            ]
        )
        # We should allow soft labels
        if len(self.targets.shape) == 1:  # hard labels
        ...
        ### For normal encoding:
        if len(self.targets.shape) == 1:  # hard labels
            self.classes = np.arange(
                self.targets.max() + 1
            )  # Jiaojiao TBD modification
        else:
            self.classes = np.arange(self.targets.shape[1])
    ...
    def open_archives(self):
        return [
            h5py.File(h5_file, "r")
            for h5_file in sorted(
                glob.glob(os.path.join(self.h5_path, f"{self.pattern}-*.hdf5"))
            )
        ]
    ...
    def __getitem__(self, index):
        ...
        if self.memory_cache:
            ...
            # y = torch.from_numpy(self.cache[a][self.target_name][i : i + self.window]).type(torch.LongTensor)
            y = torch.from_numpy(self.targets[i : i + self.window])
        else:
            archive = self.archives[a]
            X = torch.from_numpy(archive["X"][i : i + self.window])
            # y = archive[self.target_name][i : i + self.window]
            y = self.targets[index : index + self.window].numpy()  # .item()
