...
def get_vtab_dataset(vtab_data_path, ds_name, fold='train'):
    """ Get VTAB embedding dataset in HDF5 format. Genrelizes function below to any fold."""
    train_dataset = H5Dataset(
        os.path.join(vtab_data_path, NAME_MAP[ds_name], fold), pattern=f"{fold}-mae"
    )
    return train_dataset

def get_vtab_train_dataset(vtab_data_path, ds_name):
    train_dataset = H5Dataset(
        os.path.join(vtab_data_path, NAME_MAP[ds_name], "train"), pattern="train-mae"
    )
    return train_dataset
